...
# from pacer.setup import scene_name
...
class BaseTask():
    ...
    def step(self, actions):
        ...
        # time2 = time.time()
        # print(time2-time1,"self.pre_physics_step(actions)")
        # step physics and render each frame
        self._physics_step();
        # time3 = time.time()
        # print(time3-time2,"self._physics_step();")
        # to fix!
        if self.device == 'cpu':
            ...
            for evt in self.gym.query_viewer_action_events(self.viewer):
                ...
                if evt.action == "PAUSE" and evt.value > 0:
                    ...
                    if self.recording:
                        ...
                        self.viewer_record_dir = 'output/renderings/' +  curr_date_time
                        if not os.path.exists(self.viewer_record_dir):
                            os.makedirs(self.viewer_record_dir)
            ...
            if self.recording_state_change:
                if not self.recording:
                    ...
                    sample_frame = cv2.imread(
                        os.path.join(self.viewer_record_dir, images[0])
                    )
                    height, width, layers = sample_frame.shape

                    video_path = osp.join(self.viewer_record_dir, f"{self.cfg_name}_{self.scene_name}.mp4")
                    fourcc = "MP4V"
                    ...
                    for image in images:
                        video.write(
                            cv2.imread(
                                os.path.join(self.viewer_record_dir, image)
                            )
                        )
                    ...
                    video.release()
                    #delete images
                    # image_files = glob.glob(os.path.join(self.viewer_record_dir, images)) # 批量删除
                    for img in images:
                        os.remove(os.path.join(self.viewer_record_dir, img))
                            

                    states_path = osp.join(self.viewer_record_dir, f"{self.cfg_name}_{self.scene_name}.pkl")
                    self._write_states_to_file(states_path)
            ...
            if self.recording or flags.server_mode:
                ...
                if flags.no_virtual_display:
                    ...
                    self.gym.write_viewer_image_to_file(
                        self.viewer, self.viewer_record_dir + "/%04d.png" % self.stepcnt )
                    self.stepcnt += 1
                    ...
                    # # color_image = self.gym.get_camera_image(self.sim, self.envs, self.recorder_camera_handles, gymapi.IMAGE_COLOR)
                    # self.color_image = color_image.reshape(color_image.shape[0], -1, 4)
                else:
