...
class HumanoidTraj(humanoid_amp_task.HumanoidAMPTask):
    def __init__(self, cfg, sim_params, physics_engine, device_type, device_id, headless):#1

        self._num_traj_samples = cfg["env"]["numTrajSamples"]#10
        self._traj_sample_timestep = cfg["env"]["trajSampleTimestep"]#0.5
    ...
    def _build_traj_generator(self):#1
    ...
    def _build_marker_state_tensors(self):#1
        num_actors = self._root_states.shape[0] // self.num_envs#11 ?
        self._marker_states = self._root_states.view(self.num_envs, num_actors, self._root_states.shape[-1])[..., 1:(1 + self._num_traj_samples), :]#(2,10,13)
        self._marker_pos = self._marker_states[..., :3]#[2,10,13]
        ...
        self._marker_actor_ids = self._marker_actor_ids.flatten()#[20]

        return
    def refresh_map(self,root_pos):
        self.terrain.rmap = torch.zeros_like(self.terrain.rmap,dtype=torch.uint8)
        n = root_pos.shape[0]
        for i in range(n):
            x,y = self.terrain.get_index(root_pos[i])
